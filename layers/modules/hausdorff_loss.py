...
class HausdorffDTLoss(nn.Module):
    ...
    def forward(
        ...
        pred_dt = torch.from_numpy(self.distance_field(pred.cpu().detach().numpy())).float().to(self.device)
        target_dt = torch.from_numpy(self.distance_field(target.cpu().detach().numpy())).float().to(self.device)
