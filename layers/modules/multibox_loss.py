...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, net, predictions, targets, masks, num_crowds):
        ...
        if cfg.use_hausdorff_loss:
            losses['HD'] = self.hausdorff_loss(predictions['segm'], masks, labels)
    ...
    def hausdorff_loss(self, segment_data, mask_t, class_t, interpolation_mode='bilinear'):
        batch_size, num_classes, mask_h, mask_w = segment_data.size()
        loss_s = 0
        ...
        loss = self.hd_loss(segment_data, segment_t_batch)
