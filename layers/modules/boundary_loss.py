...
from .utils import simplex, probs2one_hot, one_hot, one_hot2dist, class2one_hot
...
class SurfaceLoss():
    def __init__(self, **kwargs):        # Self.idc is used to filter out some classes of the target mask. Use fancy indexing
        self.idc: List[int] = kwargs["idc"]

        self.dist_map_transform = transforms.Compose([
                lambda img: img.cpu().numpy(),
                partial(one_hot2dist, resolution=kwargs["res"]),                   
    ...
    def __call__(self, probs: Tensor, target: Tensor) -> Tensor:
        ...
        # Make Distance Maps
        dist_maps = torch.zeros_like(target, requires_grad=False)
        for i in range(dist_maps.size()[0]):
            dist_maps[i] = self.dist_map_transform(target[i])
        ...
        multipled = einsum("bkwh,bkwh->bkwh", pc, dc) # Hadamard Product
