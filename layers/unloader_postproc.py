...
def unloader_pp(det_output, h, w, top_k=15, score_threshold=0.5):
    ...
    dict_for_YolactSegm = {
        ...
        "image_size": (masks.size()[1], masks.size()[2]),
    }
...
def unld_postproc(det_output, h, w, top_k=15, score_threshold=0.5):
    # L172, see https://github.com/dbolya/yolact/blob/master/eval.py #L149
    t = postprocess(det_output, w, h, score_threshold=score_threshold)
    # L175-L178, see https://github.com/dbolya/yolact/blob/master/eval.py #L155-L160
    idx = t[1].argsort(0, descending=True)[:top_k]
    ...
    # copy mask from GPU to cpu, convert type into integer.
    masks_np = masks.to(torch.int).cpu().numpy().astype(np.uint8)

    result = []
    for i in range(len(classes)):
        result.append(
            {
                "id": (classes[i] + 1).astype(np.int32),
                "score": scores[i],
