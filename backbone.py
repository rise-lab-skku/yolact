...
class ResNetBackbone(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dcn_layers=0, dcn_interval=1):
        ...
        layers.append(block(self.inplanes, planes, stride, downsample,
                      self.norm_layer, self.dilation, use_dcn=use_dcn))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            use_dcn = ((i+dcn_layers) >= blocks) and (i % dcn_interval == 0)
            layers.append(block(self.inplanes, planes,
                          norm_layer=self.norm_layer, use_dcn=use_dcn))
    ...
    def add_layer(self, conv_channels=1024, downsample=2, depth=1, block=Bottleneck):
        ...
        self._make_layer(block, conv_channels // block.expansion,
                         blocks=depth, stride=downsample)
...
class ResNetBackboneGN(ResNetBackbone):
    ...
    def init_backbone(self, path):
        ...
        def gn_trans(x): return ('gn_s' if x == 'weight' else 'gn_b')
        def layeridx2res(x): return 'res' + str(int(x)+2)
        def block2branch(x): return 'branch2' + ('a', 'b', 'c')[int(x[-1:])-1]
...
class DarkNetBlock(nn.Module):
    ...
    def __init__(self, in_channels, channels):
        ...
        self.conv1 = darknetconvlayer(
            in_channels, channels,                  kernel_size=1)
        self.conv2 = darknetconvlayer(
            channels,    channels * self.expansion, kernel_size=3, padding=1)
...
class DarkNetBackbone(nn.Module):
    ...
    def _make_layer(self, block, channels, num_blocks, stride=2):
        ...
        layer_list += [block(self.in_channels, channels)
...
class VGGBackbone(nn.Module):
    ...
    def __init__(self, cfg, extra_args=[], norm_layers=[]):
        ...
        self.norms = nn.ModuleList(
            [nn.BatchNorm2d(self.channels[l]) for l in norm_layers])
    ...
    def _make_layer(self, cfg):
        ...
        for v in cfg:
            ...
            if v == 'M':
                ...
                self.state_dict_lookup[cur_layer_idx] = '%d.%d' % (
                    len(self.layers), len(layers))
    ...
    def init_backbone(self, path):
        """ Initializes the backbone weights for training. """
        state_dict = torch.load(path)
        state_dict = OrderedDict([(self.transform_key(k), v)
    ...
    def add_layer(self, conv_channels=128, downsample=2):
        ...
        layer = nn.Sequential(
            ...
            nn.Conv2d(conv_channels, conv_channels*2, kernel_size=3,
